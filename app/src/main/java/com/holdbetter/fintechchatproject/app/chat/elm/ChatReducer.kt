    ...
    override fun Result.reduce(event: ChatEvent): Any {
        return when (event) {
            ...
            is ChatEvent.Ui.ReactionSent -> {
                commands { +ChatCommand.SendReaction(event.messageId, event.reaction, event.currentMessages) }
            }
            is ChatEvent.Ui.ReactionRemoved -> {
                commands { +ChatCommand.RemoveReaction(event.messageId, event.reaction, event.currentMessages) }
            }
