...
class StreamChatRepository @AssistedInject constructor(
...
) : BaseChatRepository(connectivityManager, api) {
    private val streamNarrow = Narrow.MessageNarrow(streamId).toJson()

    override val jsonNarrow: String
        get() = streamNarrow

    override fun getCachedMessages(): Maybe<List<MessageItem.Message>> {
        return messageDao.getStreamMessages(streamId)
    ...
    override fun getFirstPortion(): Single<Pair<Boolean, List<MessageItem.Message>>> {
        return connectivityManager.isConnected
            ...
            .flatMap { getApi(it) }
            .flatMap { api -> api.getNewestMessages(streamNarrow) }
            .map { it.toMessage() }
    ...
    override fun getNextPortion(
        ...
        return connectivityManager.isConnected
            .subscribeOn(Schedulers.io())
            .flatMap { getApi(it) }
            .flatMap { api -> api.getNextPortion(streamNarrow, messageAnchorId) }
            .map { it.toMessage() }
...
@AssistedFactory
interface StreamChatRepositoryFactory {
    fun create(
